...
if [ "$OS" = "Darwin" ]; then
  ...
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -Xdock:icon="../Resources/ZAP.icns" -jar "${BASEDIR}/zap-2.14.0.jar" "${ARGS[@]}"
else
  # default line coming in with the standard package installation doesnt support html viewer 
  #exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -jar "${BASEDIR}/zap-2.14.0.jar" "${ARGS[@]}"
  # this line remove the problem of javaFX but it require that javafx installed in the system 
  exec java --module-path /usr/share/openjfx/lib --add-modules=javafx.base,javafx.controls,javafx.fxml,javafx.graphics,javafx.media,javafx.swing,javafx.web ${JMEM} ${JAVAGC} ${JAVADEBUG} -jar "${BASEDIR}/zap-2.14.0.jar" "${ARGS[@]}"
fi
